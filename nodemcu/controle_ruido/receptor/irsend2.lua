...
do
  ...
  local NEC_HDR_MARK   = 4000
  local NEC_HDR_SPACE  = 4500
  local NEC_BIT_MARK   =  406
  local NEC_ONE_SPACE  = 1780
  local NEC_ZERO_SPACE =  670
  ...
  local nec = function(pin, code1, code2, tsop)
    ...
    for i = 15, 0, -1 do
      ...
      waitus(isset(code1, i) and NEC_ONE_SPACE or NEC_ZERO_SPACE)
    end
    
    for i = 31, 0, -1 do
      pulse(pin, NEC_BIT_MARK)
      waitus(isset(code2, i) and NEC_ONE_SPACE or NEC_ZERO_SPACE)
    end
    -- trailer
    pulse(pin, NEC_BIT_MARK)
    -- done transmitter
